...
public class InstituicaoEndpoint {
	...
	public Response create(Instituicao entity) {
		em.persist(entity);
		return Response.created(
				UriBuilder.fromResource(InstituicaoEndpoint.class)
	...
	public Response findById(@PathParam("id") Long id) {
		TypedQuery<Instituicao> findByIdQuery = em
				.createQuery(
						"SELECT DISTINCT i FROM Instituicao i LEFT JOIN FETCH i.endereco WHERE i.id = :entityId ORDER BY i.id",
						Instituicao.class);
	...
	public List<Instituicao> listAll(
		...
		final List<Instituicao> results = findAllQuery.getResultList();
		return results;
	}
	...
	@Path("/{id:[0-9][0-9]*}")
	@Consumes("application/xml")
	public Response update(@PathParam("id") Long id, Instituicao entity) {
		if (entity == null) {
		...
		if (em.find(Instituicao.class, id) == null) {
			return Response.status(Status.NOT_FOUND).build();
		}
		try {
